...
function PCParts() {
  ...
  // Dynamic selected item based on loaded categories
  ...
  // Load categories dynamically from API on component mount
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const response = await kioskAPI.getCategories();
        console.log('📊 Categories API response:', response);
        
        const categoriesData = response?.data || response || [];
        
        // Map categories to include proper images and exclude Monitor from sidebar
        ...
        // Add regular categories excluding Monitor and grouping peripherals
        categoriesData.forEach(cat => {
          if (cat.category === 'Monitor') return; // Skip Monitor in sidebar
          
          // Map category to proper display
          const categoryMapping = {
            'CPU': { name: 'Central Processing Unit', image: CPU1 },
            'GPU': { name: 'Graphics Processing Unit', image: GPU1 },
            'Motherboard': { name: 'Motherboard', image: Motherboard1 },
            'RAM': { name: 'Memory (RAM)', image: Ram },
            'Storage': { name: 'Storage', image: Storage1 },
            'PSU': { name: 'Power Supply Unit', image: Frame },
            'Case': { name: 'PC Case', image: Chest },
            'Cooling': { name: 'CPU Cooler', image: COOLMANREYNA }, // Fixed: CPU Cooler shows cooling products
          };
          
          const mapping = categoryMapping[cat.category];
          if (mapping) {
        ...
        // Add Peripherals as grouped category
        const peripheralCategories = categoriesData.filter(cat => 
          ['Monitor', 'Keyboard', 'Mouse', 'Headphones', 'Speakers', 'Webcam'].includes(cat.category)
        );
        
        if (peripheralCategories.length > 0) {
          mappedCategories.push({
            category: 'Peripherals',
            name: 'Peripherals',
            image: GPU1, // Using GPU1 as peripherals placeholder
            productCount: peripheralCategories.reduce((sum, cat) => sum + (cat.productCount || 0), 0),
            subCategories: peripheralCategories.map(cat => ({
              ...cat,
              name: cat.category // Keep original category name for subcategories
            })),
            order: 150
          });
        }
        ...
        // Sort by order
        mappedCategories.sort((a, b) => (a.order || 999) - (b.order || 999));
        ...
        // Fallback to static menuItems if API fails
        setCategories(menuItems.slice(0, 9)); // Exclude monitor from static fallback
      } finally {
        setLoading(false);
      }
    };
  ...
  // Enhanced category products loading with peripheral support
  useEffect(() => {
    const loadCategoryProducts = async () => {
      if (!categories.length || selectedItem < 0 || selectedItem >= categories.length) return;
      ...
      try {
        setLoading(true);
        console.log('🔍 Loading products for:', selectedCategory);
        
        let categoryToFetch = selectedCategory.category;
        
        // Handle peripheral subcategory selection
        if (selectedCategory.category === 'Peripherals') {
          if (selectedPeripheralSubCategory) {
            categoryToFetch = selectedPeripheralSubCategory;
          } else if (selectedCategory.subCategories?.length > 0) {
            // Default to first subcategory
            const firstSubCat = selectedCategory.subCategories[0];
            categoryToFetch = firstSubCat.category;
            setSelectedPeripheralSubCategory(firstSubCat.category);
          }
        }
        
        console.log('📦 Fetching products for category:', categoryToFetch);
        
        const response = await kioskAPI.getCategoryProducts(categoryToFetch, { 
          limit: 100,
          sortBy,
          sortOrder,
        ...
        const transformedProducts = fetchedProducts.map(product => ({
          ...
          image: product.imageUrl || product.image || getDefaultCategoryImage(categoryToFetch),
          details: product.description || 'No description available',
          specifications: formatProductSpecifications(product.specifications),
        ...
        const brands = [...new Set(transformedProducts.map(p => p.brand).filter(Boolean))].sort();
        setAvailableBrands(brands);
        
        // Set peripheral subcategories if needed
        ...
        console.log(`✅ Loaded ${transformedProducts.length} products, ${brands.length} brands`);
  ...
  }, [categories, selectedItem, selectedPeripheralSubCategory, sortBy, sortOrder, selectedBrand, priceRange]);
  ...
  // Enhanced specifications formatter
  const formatProductSpecifications = (specs) => {
  ...
  const handlePeripheralSubCategoryClick = (subCategory) => {
  ...
  const handleCategoryFilterChange = (filterKey, filterValue) => {
    setCategoryFilters(prev => ({
      ...prev,
      [filterKey]: filterValue === prev[filterKey] ? undefined : filterValue
    }));
  };
  ...
  // Get available category-specific filter options
  const getCategoryFilters = () => {
    if (!categories[selectedItem] || categories[selectedItem].category === 'home') return {};
    
    const category = categories[selectedItem].category;
    const categoryToCheck = selectedPeripheralSubCategory || category;
    
    // Define filters based on category specifications
    const filterDefinitions = {
      'CPU': {
        'series': { label: 'Series', options: ['Core i3', 'Core i5', 'Core i7', 'Core i9', 'Ryzen 3', 'Ryzen 5', 'Ryzen 7', 'Ryzen 9'] },
        'socket': { label: 'Socket', options: ['AM4', 'AM5', 'LGA1700', 'LGA1200'] },
        'cores': { label: 'Cores', options: ['4', '6', '8', '12', '16'] }
      },
      'GPU': {
        'memory_capacity': { label: 'Memory', options: ['4GB', '6GB', '8GB', '12GB', '16GB', '24GB'] },
        'memory_type': { label: 'Memory Type', options: ['GDDR6', 'GDDR6X'] }
      },
      'Motherboard': {
        'socket': { label: 'Socket', options: ['AM4', 'AM5', 'LGA1700', 'LGA1200'] },
        'chipset': { label: 'Chipset', options: ['B450', 'B550', 'X570', 'B460', 'B560', 'Z590'] }
      },
      'RAM': {
        'memory_type': { label: 'Memory Type', options: ['DDR4', 'DDR5'] },
        'speed': { label: 'Speed', options: ['2400', '2666', '3200', '3600', '4800', '5600'] },
        'total_capacity': { label: 'Capacity', options: ['8', '16', '32', '64'] }
      },
      'Storage': {
        'storage_type': { label: 'Type', options: ['SSD', 'HDD', 'NVMe'] },
        'capacity': { label: 'Capacity', options: ['256GB', '500GB', '1TB', '2TB'] }
      },
      'PSU': {
        'wattage': { label: 'Wattage', options: ['450', '550', '650', '750', '850', '1000'] },
        'efficiency_rating': { label: 'Efficiency', options: ['80+ Bronze', '80+ Gold', '80+ Platinum'] }
      },
      'Cooling': {
        'water_cooled': { label: 'Type', options: ['Air Cooler', 'AIO Liquid'] }
      },
      'Monitor': {
        'resolution': { label: 'Resolution', options: ['1920x1080', '2560x1440', '3840x2160'] },
        'refresh_rate': { label: 'Refresh Rate', options: ['60Hz', '75Hz', '100Hz', '144Hz', '240Hz'] }
      }
    };
  ...
  const filteredProducts = React.useMemo(() => {
      ...
      if (value) {
        filtered = filtered.filter(product => {
          ...
          if (typeof specs === 'string') {
            return specs.toLowerCase().includes(value.toLowerCase());
          }
          if (typeof specs === 'object' && specs[key]) {
            return String(specs[key]).toLowerCase().includes(value.toLowerCase());
          }
          return false;
        });
      }
    ...
    return filtered;
  }, [allProducts, selectedBrand, priceRange, categoryFilters, sortBy, sortOrder]);
  ...
  useEffect(() => {
    const loadFeaturedProducts = async () => {
      try {
        ...
        const onSaleResponse = await kioskAPI.getOnSaleProducts();
        console.log('🏷️ On-sale API response:', onSaleResponse);
        
        const onSaleProducts = Array.isArray(onSaleResponse) 
          ? onSaleResponse 
          : (onSaleResponse?.data || []);
          
        if (onSaleProducts.length > 0) {
          setRandomOnSale(onSaleProducts.slice(0, 5));
        } else {
          console.log('⚠️ No on-sale products, using fallback');
          setRandomOnSale([]);
        }

        // Set other featured products (these could also be API-based later)
        setRandomHotPicks([]);
  ...
  //Function for Product-List (Enhanced with proper specifications handling)
  const handleProductClick = (category, product, index) => {
    const safeCategory = category || categories[selectedItem]?.category || "unknown-category";
    ...
    // Enhanced specifications handling - ensure it's a string
    let specifications = "No specifications provided.";
  ...
  const handleSpecChange = (specKey, value) => {
    setSelectedSpecs(prev => ({
      ...prev,
      [specKey]: value === '' ? undefined : value
    }));
  };

  const handleApplyFilters = () => {
    // This will trigger the useEffect that filters products
    // The filtering logic is already in the useEffect that depends on these state variables
  ...
  return (
    <div className="pc-parts-container">
      ...
      <div className="main-content">
        {categories[selectedItem]?.category === "home" ? (
          ...
          // Product List (Enhanced with dynamic categories and peripheral support)
          <>
            {/* Product Category Header */}
            <div className="category-header">
              <h2 className="category-title">
                {loading ? 'Loading...' : (categories[selectedItem]?.name || 'Unknown Category')}
              </h2>
              <span className="category-count">[{products.length}]</span>
            </div>

            {/* Peripheral Subcategory Selection */}
                ...
                <div className="subcategory-section">
                  ...
                  <div className="subcategory-list">
                    {peripheralSubCategories.map(subCat => (
                      ...
                      >
                        {subCat.name} ({subCat.productCount || 0})
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="brand-filter-container">
              <div className="brand-section">
                ...
                <div className="brand-list">
                  ...
                  >
                    <span>All Brands ({allProducts.length})</span>
                  </div>
              ...
              <div className="grid">
                {products && products.length > 0 ? (
                  ...
                  <div style={{ padding: "20px", textAlign: "center", color: "white" }}>
                    <p>
                      {selectedBrand 
                        ? `No products found matching your criteria. (Brand: ${selectedBrand})`
      ...
      {showFilter && (
        <div className="popup-overlay" onClick={handleOutsideClick}>
          <div className="popup" onClick={(e) => e.stopPropagation()}>
            ...
            <button className="clear-all" onClick={handleClearFilters}>Clear All</button>

            {/* Manufacturer/Brand Filter */}
            <div className="filter-section">
              <div className="filter-header">
                <span>BRAND</span>
                <button onClick={() => setBrandFilterOpen(!brandFilterOpen)} className="toggle-button">
                  {brandFilterOpen ? "−" : "+"}
                </button>
              </div>
              {brandFilterOpen && (
                <div className="filter-options">
                  <button 
                    className={`filter-btn ${selectedBrands.length === 0 ? 'active' : ''}`} 
            ...
            <div className="filter-section">
              <div className="filter-header">
                ...
                <button onClick={() => setPriceFilterOpen(!priceFilterOpen)} className="toggle-button">
                  {priceFilterOpen ? "−" : "+"}
                </button>
              </div>
              {priceFilterOpen && (
            ...
            <div className="filter-section">
              <div className="filter-header">
                ...
                <button onClick={() => setNameSortOpen(!nameSortOpen)} className="toggle-button">
                  {nameSortOpen ? "−" : "+"}
                </button>
              </div>
              {nameSortOpen && (
                <div className="filter-options">
                  <button 
                    className={`filter-btn ${nameSort === 'none' ? 'active' : ''}`}
                    onClick={() => setNameSort('none')}
                  >
                    DEFAULT
                  </button>
                  <button 
                    className={`filter-btn ${nameSort === 'asc' ? 'active' : ''}`}
                    onClick={() => setNameSort('asc')}
                  >
                    A-Z
                  </button>
                  <button 
                    className={`filter-btn ${nameSort === 'desc' ? 'active' : ''}`}
            ...
            {currentCategory && availableSpecs.length > 0 && (
              <div className="filter-section">
                <div className="filter-header">
                  <span>SPECIFICATIONS</span>
                  <button onClick={() => setSpecsFilterOpen(!specsFilterOpen)} className="toggle-button">
                    {specsFilterOpen ? "−" : "+"}
                  </button>
                </div>
                {specsFilterOpen && (
