    ...
    const fetchQueueData = async () => {
        try {
            ...
            if (response.data && response.data.success) {
                ...
                // Filter for display (active queues with orders)
                ...
                displayQueues.sort((a, b) => a.queue_number - b.queue_number);

                setQueueData(displayQueues);

                // Find current serving (processing or ready)
                const serving = displayQueues.find(item => 
                    item.queue_status === 'processing' || item.queue_status === 'ready'
                );

                setCurrentQueue(serving || displayQueues[0] || null);
        ...
        <div className="queue-display-container">
            ...
            <div className="queue-list-section">
                ...
                <div className="queue-list">
                    {queueData
                        .filter(item => item.queue_number !== currentQueue?.queue_number) // Exclude current queue
                        .slice(0, 8) // Show max 8 queues
                        .map((item, index) => (
                            <div key={item.queue_number} className={`queue-item ${getStatusColor(item.queue_status)}`}>
                                ...
                                <div className="order-details">
                                    ...
                                    <div className="order-status-small">
                                        ...
                                        <span>{getEstimatedTime(item, index)}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
