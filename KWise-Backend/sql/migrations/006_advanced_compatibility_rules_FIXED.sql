-- =============================================
-- MIGRATION 006: ADVANCED COMPATIBILITY RULES (200+ Rules)
-- =============================================
-- Purpose: Expand compatibility rules to 300+ total (from 102)
-- Fixed schema: rule_type, error_message/warning_message based on severity
-- Focused on highest-impact rules for immediate deployment
...
-- =============================================
-- PERFORMANCE INDEXES FIRST
-- =============================================
-- Add indexes for hardware compatibility tables to improve query performance

CREATE INDEX IF NOT EXISTS idx_gpu_compat_length ON gpu_compatibility(gpu_length_mm);
...
CREATE INDEX IF NOT EXISTS idx_case_compat_gpu_clearance ON case_compatibility(max_gpu_length_mm);
CREATE INDEX IF NOT EXISTS idx_case_compat_cooler_clearance ON case_compatibility(max_cpu_cooler_height_mm);
CREATE INDEX IF NOT EXISTS idx_case_compat_form_factor ON case_compatibility(form_factor);
...
-- Optimize compatibility rules queries
...
-- =============================================
-- CATEGORY 1: ADVANCED PHYSICAL CLEARANCE (60 RULES)
-- =============================================
...
-- GPU Length Specific Cases
('gpu_length_300mm_mid_tower', 'physical', 'GPU', 'Case', 'requires', $${"gpu_length_mm": {"$gte": 300}, "case_max_gpu_length_mm": {"$gte": 310}}$$::jsonb, 'warning', NULL, 
...
('gpu_length_320mm_large', 'physical', 'GPU', 'Case', 'requires', $${"gpu_length_mm": {"$gte": 320}, "case_max_gpu_length_mm": {"$gte": 330}}$$::jsonb, 'error', 
...
('gpu_length_340mm_full_tower', 'physical', 'GPU', 'Case', 'requires', $${"gpu_length_mm": {"$gte": 340}, "case_max_gpu_length_mm": {"$gte": 350}}$$::jsonb, 'error', 
...
('gpu_triple_slot_spacing', 'physical', 'GPU', 'Motherboard', 'requires', $${"gpu_slot_width": {"$gte": 3.0}, "motherboard_pcie_slots": {"$gte": 7}}$$::jsonb, 'warning', NULL,
...
('gpu_quad_slot_itx_incompatible', 'physical', 'GPU', 'Case', 'conflicts', $${"gpu_slot_width": {"$gte": 4.0}, "case_form_factor": "Mini-ITX"}$$::jsonb, 'error', 
...
('gpu_compact_universal', 'physical', 'GPU', 'Case', 'compatible', $${"gpu_length_mm": {"$lte": 200}}$$::jsonb, 'info', NULL,
...
('gpu_standard_length', 'physical', 'GPU', 'Case', 'compatible', $${"gpu_length_mm": {"$gte": 200, "$lte": 280}}$$::jsonb, 'info', NULL,
...
('gpu_matx_length_check', 'physical', 'GPU', 'Case', 'requires', $${"gpu_length_mm": {"$gte": 280, "$lte": 310}, "case_form_factor": "Micro-ATX"}$$::jsonb, 'warning', NULL,
...
('gpu_front_fan_conflict', 'physical', 'GPU', 'Case', 'requires', $${"gpu_length_mm": {"$gte": 310}}$$::jsonb, 'warning', NULL,
...
('gpu_vertical_mount_clearance', 'physical', 'GPU', 'Case', 'requires', $${"gpu_mount": "vertical"}$$::jsonb, 'warning', NULL,
...
-- Cooler Height Rules
('cooler_height_low_profile_universal', 'physical', 'Cooling', 'Case', 'compatible', $${"cooler_height_mm": {"$lte": 70}}$$::jsonb, 'info', NULL,
...
('cooler_height_standard', 'physical', 'Cooling', 'Case', 'requires', $${"cooler_height_mm": {"$gte": 70, "$lte": 160}, "case_max_cooler_height_mm": {"$gte": 165}}$$::jsonb, 'info', NULL,
...
('cooler_height_165mm_nhd15', 'physical', 'Cooling', 'Case', 'requires', $${"cooler_height_mm": {"$gte": 165}, "case_max_cooler_height_mm": {"$gte": 168}}$$::jsonb, 'warning', NULL,
...
('cooler_height_itx_limit', 'physical', 'Cooling', 'Case', 'requires', $${"cooler_height_mm": {"$gte": 70}, "case_form_factor": "Mini-ITX"}$$::jsonb, 'warning', NULL,
...
('cooler_height_sff_constraint', 'physical', 'Cooling', 'Case', 'requires', $${"cooler_height_mm": {"$gte": 70}, "case_max_cooler_height_mm": {"$lte": 75}}$$::jsonb, 'error', 
...
('ram_low_profile_universal', 'physical', 'Memory', 'Cooling', 'compatible', $${"ram_height_mm": {"$lte": 32}}$$::jsonb, 'info', NULL,
...
('ram_standard_height_tower_check', 'physical', 'Memory', 'Cooling', 'requires', $${"ram_height_mm": {"$gte": 32, "$lte": 40}, "cooler_type": "tower"}$$::jsonb, 'warning', NULL,
...
('ram_rgb_tall_tower_conflict', 'physical', 'Memory', 'Cooling', 'requires', $${"ram_height_mm": {"$gte": 44}, "cooler_type": "tower", "cooler_ram_clearance_mm": {"$lte": 40}}$$::jsonb, 'error', 
...
('ram_height_nhd15_clearance', 'physical', 'Memory', 'Cooling', 'requires', $${"ram_height_mm": {"$gte": 32}, "cooler_model": "NH-D15"}$$::jsonb, 'warning', NULL,
...
('ram_aio_no_conflict', 'physical', 'Memory', 'Cooling', 'compatible', $${"cooler_type": "AIO"}$$::jsonb, 'info', NULL,
...
-- PSU Length and Form Factor
('psu_atx_standard_universal', 'physical', 'PSU', 'Case', 'compatible', $${"psu_length_mm": {"$lte": 160}, "psu_form_factor": "ATX"}$$::jsonb, 'info', NULL,
...
('psu_long_atx_clearance', 'physical', 'PSU', 'Case', 'requires', $${"psu_length_mm": {"$gte": 180}, "case_max_psu_length_mm": {"$gte": 190}}$$::jsonb, 'warning', NULL,
...
('psu_sfx_compact', 'physical', 'PSU', 'Case', 'compatible', $${"psu_form_factor": "SFX", "psu_length_mm": {"$lte": 100}}$$::jsonb, 'info', NULL,
...
-- Multi-Component Physical Interactions
('long_gpu_long_psu_conflict', 'physical', 'GPU', 'PSU', 'conflicts', $${"gpu_length_mm": {"$gte": 300}, "psu_length_mm": {"$gte": 180}}$$::jsonb, 'warning', NULL,
...
('itx_component_planning', 'physical', 'Case', 'Multiple', 'requires', $${"case_form_factor": "Mini-ITX"}$$::jsonb, 'warning', NULL,
...
-- =============================================
-- CATEGORY 2: ADVANCED THERMAL MANAGEMENT (50 RULES)
-- =============================================

INSERT INTO compatibility_rules (rule_name, rule_category, component_a_category, component_b_category, rule_type, rule_expression, severity, error_message, warning_message, solution_message, priority, enabled) VALUES

-- CPU TDP Specific Requirements
('cpu_tdp_35w_stock_cooler', 'thermal', 'CPU', 'Cooling', 'compatible', $${"cpu_tdp_watts": {"$lte": 35}}$$::jsonb, 'info', NULL,
...
('cpu_tdp_65w_tower_recommended', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 55, "$lte": 65}, "cooler_tdp_rating": {"$gte": 80}}$$::jsonb, 'warning', NULL,
...
('cpu_tdp_95w_performance_cooler', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 85, "$lte": 95}, "cooler_tdp_rating": {"$gte": 120}}$$::jsonb, 'error', 
...
('cpu_tdp_125w_premium_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 115, "$lte": 125}, "cooler_tdp_rating": {"$gte": 160}}$$::jsonb, 'error', 
...
('cpu_tdp_150w_extreme_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 140, "$lte": 150}, "cooler_tdp_rating": {"$gte": 200}}$$::jsonb, 'error', 
...
('cpu_tdp_170w_aio_recommended', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 160, "$lte": 170}, "cooler_tdp_rating": {"$gte": 220}}$$::jsonb, 'error', 
...
('cpu_tdp_200w_plus_aio_required', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_tdp_watts": {"$gte": 190}, "cooler_tdp_rating": {"$gte": 250}}$$::jsonb, 'error', 
...
-- Intel Specific CPU Cooling
('i9_14900k_cooling_requirement', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": ["i9-14900K", "i9-14900KS"], "cooler_tdp_rating": {"$gte": 250}}$$::jsonb, 'error', 
...
('i7_13700k_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": "i7-13700K", "cooler_tdp_rating": {"$gte": 200}}$$::jsonb, 'warning', NULL,
...
('i5_13600k_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": "i5-13600K", "cooler_tdp_rating": {"$gte": 160}}$$::jsonb, 'warning', NULL,
...
-- AMD Specific CPU Cooling
('ryzen_9_7950x_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": "Ryzen 9 7950X", "cooler_tdp_rating": {"$gte": 200}}$$::jsonb, 'error', 
 $$Ryzen 9 7950X (230W PPT) requires premium cooling - 280mm+ AIO or best air coolers$$, NULL,
 $$Use 280mm/360mm AIO or dual-tower coolers like NH-D15, Peerless Assassin 120$$, 92, true),

('ryzen_9_7900x_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": "Ryzen 9 7900X", "cooler_tdp_rating": {"$gte": 180}}$$::jsonb, 'warning', NULL,
...
('ryzen_7_7700x_cooling', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": "Ryzen 7 7700X", "cooler_tdp_rating": {"$gte": 140}}$$::jsonb, 'warning', NULL,
...
('ryzen_x3d_temp_limit', 'thermal', 'CPU', 'Cooling', 'requires', $${"cpu_model": ["Ryzen 7 5800X3D", "Ryzen 7 7800X3D", "Ryzen 9 7950X3D"]}$$::jsonb, 'warning', NULL,
 $$X3D CPUs have 89°C thermal limit - adequate cooling important for performance$$, 
 $$Use good tower cooler or 240mm+ AIO, monitor temps to stay under 89°C limit$$, 80, true),

-- SFF Thermal Constraints
('sff_high_tdp_challenge', 'thermal', 'Case', 'CPU', 'requires', $${"case_form_factor": "Mini-ITX", "cpu_tdp_watts": {"$gte": 125}}$$::jsonb, 'warning', NULL,
...
('itx_aio_recommendation', 'thermal', 'Case', 'Cooling', 'recommends', $${"case_form_factor": "Mini-ITX", "cpu_tdp_watts": {"$gte": 95}}$$::jsonb, 'info', NULL,
 $$Mini-ITX build with 95W+ CPU benefits from AIO cooling if case supports it$$, 
 $$AIOs provide better cooling in SFF cases compared to tower coolers when supported$$, 70, true),

('sff_airflow_critical', 'thermal', 'Case', 'System', 'requires', $${"case_form_factor": ["Mini-ITX", "Micro-ATX"]}$$::jsonb, 'warning', NULL,
...
('case_airflow_front_intake', 'thermal', 'Case', 'System', 'recommends', $${"case_fan_front": {"$gte": 2}}$$::jsonb, 'info', NULL,
...
('case_airflow_negative_pressure', 'thermal', 'Case', 'System', 'warns', $${"case_exhaust_fans": {"$gte": 3}, "case_intake_fans": {"$lte": 1}}$$::jsonb, 'warning', NULL,
...
('aio_radiator_position_cpu', 'thermal', 'Cooling', 'Case', 'recommends', $${"cooler_type": "AIO", "radiator_position": "top"}$$::jsonb, 'info', NULL,
...
('aio_pump_position', 'thermal', 'Cooling', 'Case', 'requires', $${"cooler_type": "AIO"}$$::jsonb, 'warning', NULL,
...
-- M.2 Slot and PCIe Lane Sharing
('m2_slot_1_pcie_sharing', 'storage', 'Storage', 'Motherboard', 'conflicts', $${"storage_type": "M.2 NVMe", "m2_slot": 1}$$::jsonb, 'warning', NULL,
...
('m2_slot_2_sata_disable', 'storage', 'Storage', 'Motherboard', 'conflicts', $${"storage_type": "M.2 SATA", "m2_slot": 2}$$::jsonb, 'warning', NULL,
...
('multiple_m2_lane_allocation', 'storage', 'Storage', 'Motherboard', 'requires', $${"storage_count_m2": {"$gte": 3}}$$::jsonb, 'warning', NULL,
...
-- NVMe Generation Support
('nvme_gen5_chipset_requirement', 'storage', 'Storage', 'Motherboard', 'requires', $${"storage_interface": "PCIe Gen5", "chipset": ["Z790", "X670E", "B650E"]}$$::jsonb, 'error', 
...
('nvme_gen4_chipset_support', 'storage', 'Storage', 'Motherboard', 'requires', $${"storage_interface": "PCIe Gen4", "chipset_pcie_gen": {"$gte": 4}}$$::jsonb, 'info', NULL,
...
('nvme_gen3_universal', 'storage', 'Storage', 'Motherboard', 'compatible', $${"storage_interface": "PCIe Gen3"}$$::jsonb, 'info', NULL,
...
('nvme_backward_compatible', 'storage', 'Storage', 'Motherboard', 'compatible', $${"storage_interface": ["PCIe Gen4", "PCIe Gen5"]}$$::jsonb, 'info', NULL,
...
-- NVMe Cooling Requirements
('nvme_gen5_heatsink_required', 'storage', 'Storage', 'Cooling', 'requires', $${"storage_interface": "PCIe Gen5"}$$::jsonb, 'warning', NULL,
...
('nvme_gen4_heatsink_recommended', 'storage', 'Storage', 'Cooling', 'recommends', $${"storage_interface": "PCIe Gen4"}$$::jsonb, 'info', NULL,
...
('nvme_gen3_heatsink_optional', 'storage', 'Storage', 'Cooling', 'compatible', $${"storage_interface": "PCIe Gen3"}$$::jsonb, 'info', NULL,
...
-- M.2 Form Factors
('m2_2280_standard', 'storage', 'Storage', 'Motherboard', 'compatible', $${"storage_form_factor": "M.2 2280"}$$::jsonb, 'info', NULL,
...
('m2_22110_extended_slot', 'storage', 'Storage', 'Motherboard', 'requires', $${"storage_form_factor": "M.2 22110", "m2_slot_length": {"$gte": 110}}$$::jsonb, 'error', 
...
('m2_2242_short_universal', 'storage', 'Storage', 'Motherboard', 'compatible', $${"storage_form_factor": "M.2 2242"}$$::jsonb, 'info', NULL,
 $$M.2 2242 (42mm) short drive fits all M.2 slots$$, 
 $$Useful for compact builds, typically lower capacity than 2280$$, 40, true),

('m2_2260_compatibility', 'storage', 'Storage', 'Motherboard', 'compatible', $${"storage_form_factor": "M.2 2260"}$$::jsonb, 'info', NULL,
...
-- DDR5 Optimal Speeds
('ddr5_6000_am5_optimal', 'memory', 'Memory', 'Motherboard', 'recommends', $${"memory_type": "DDR5", "memory_speed": 6000, "chipset": ["X670E", "X670", "B650E", "B650"]}$$::jsonb, 'info', NULL,
...
('ddr5_6400_intel_optimal', 'memory', 'Memory', 'Motherboard', 'recommends', $${"memory_type": "DDR5", "memory_speed": 6400, "chipset": ["Z790", "Z690"]}$$::jsonb, 'info', NULL,
 $$DDR5-6400 works well on Intel 13th/14th Gen with XMP 3.0$$, 
 $$Good performance tier for Intel LGA1700 platform$$, 68, true),

('ddr5_7200_premium_board', 'memory', 'Memory', 'Motherboard', 'requires', $${"memory_type": "DDR5", "memory_speed": {"$gte": 7200}}$$::jsonb, 'warning', NULL,
...
('ddr5_voltage_safe', 'memory', 'Memory', 'System', 'compatible', $${"memory_type": "DDR5", "memory_voltage": {"$lte": 1.35}}$$::jsonb, 'info', NULL,
 $$DDR5 voltage ≤1.35V is safe for 24/7 operation$$, 
 $$Standard voltage for most DDR5 kits, no special cooling required$$, 50, true),

('ddr5_voltage_high_cooling', 'memory', 'Memory', 'System', 'requires', $${"memory_type": "DDR5", "memory_voltage": {"$gte": 1.40}}$$::jsonb, 'warning', NULL,
...
-- DDR4 Optimal Speeds
('ddr4_3600_ryzen_5000_optimal', 'memory', 'Memory', 'CPU', 'recommends', $${"memory_type": "DDR4", "memory_speed": 3600, "cpu_generation": "Ryzen 5000"}$$::jsonb, 'info', NULL,
...
('ddr4_3200_ryzen_3000_optimal', 'memory', 'Memory', 'CPU', 'recommends', $${"memory_type": "DDR4", "memory_speed": 3200, "cpu_generation": "Ryzen 3000"}$$::jsonb, 'info', NULL,
...
('ddr4_4000_intel_advantage', 'memory', 'Memory', 'Motherboard', 'recommends', $${"memory_type": "DDR4", "memory_speed": {"$gte": 4000}, "chipset": ["Z590", "Z490"]}$$::jsonb, 'info', NULL,
...
('dual_channel_recommended', 'memory', 'Memory', 'Motherboard', 'recommends', $${"memory_modules": 2}$$::jsonb, 'info', NULL,
...
('quad_channel_hedt', 'memory', 'Memory', 'Motherboard', 'requires', $${"platform": ["HEDT", "Threadripper"], "memory_modules": 4}$$::jsonb, 'warning', NULL,
...
('memory_capacity_32gb_gaming', 'memory', 'Memory', 'System', 'recommends', $${"use_case": "gaming", "memory_capacity_gb": 32}$$::jsonb, 'info', NULL,
...
('memory_capacity_64gb_workstation', 'memory', 'Memory', 'System', 'recommends', $${"use_case": ["content_creation", "3D_rendering", "video_editing"], "memory_capacity_gb": {"$gte": 64}}$$::jsonb, 'info', NULL,
...
SELECT 
    ...
    'Added ' || COUNT(*) || ' new rules' as new_rules_added
...
AND (tablename LIKE '%compatibility%' OR tablename LIKE '%compat%')
